...
import { Bike, Copy, LoaderCircle, Mail, MapPin, Phone, Plus, Power, Send, Truck, UserRound, X } from "lucide-react";
...
export function RestaurantDriversClient({ initialDrivers, initialInvites }: { initialDrivers: DriverRow[]; initialInvites: InviteRow[] }) {
  ...
  function toggleDriver(driver: DriverRow) {
    startTransition(async () => {
      ...
      if (!result.ok) toast.error(result.message);
        ...
        toast.success(result.message);
        setDrivers((current) => current.map((item) => item.id === driver.id ? { ...item, is_active: !driver.is_active, status: !driver.is_active ? "offline" : "suspended" } : item));
      }
    });
  }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <section className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        <div><p className="text-sm font-medium text-amber-600">Rede de entregas</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Estafetas</h1><p className="mt-2 text-sm text-zinc-500">Convide por link, acompanhe disponibilidade e controle acessos.</p></div>
      ...
      {pendingInvites.length > 0 && <Card className="border-violet-200 bg-violet-50/40 shadow-none"><CardHeader><CardTitle className="text-lg">Convites pendentes</CardTitle></CardHeader><CardContent className="space-y-3">{pendingInvites.map((invite) => { const link = `/driver/invite/${invite.token}`; return <div key={invite.id} className="flex flex-col justify-between gap-3 rounded-2xl bg-white p-4 sm:flex-row sm:items-center"><div><p className="flex items-center gap-2 font-medium"><Mail className="size-4" />{invite.email}</p><p className="mt-1 text-xs text-zinc-500">Expira em {new Intl.DateTimeFormat("pt-PT", { day: "2-digit", month: "short" }).format(new Date(invite.expires_at))}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => copyLink(link)}><Copy className="mr-1 size-4" /> Copiar</Button><Button variant="outline" size="sm" className="text-red-600" onClick={() => cancelInvite(invite.id)}><X className="mr-1 size-4" /> Cancelar</Button></div></div>; })}</CardContent></Card>}

      {!drivers.length ? <div className="flex min-h-72 flex-col items-center justify-center rounded-3xl border border-dashed border-zinc-300 bg-white text-center"><Bike className="size-9 text-zinc-300" /><p className="mt-4 font-medium">Ainda não existem estafetas</p><p className="mt-1 max-w-sm text-sm text-zinc-500">Crie um convite e envie o link ao primeiro estafeta.</p></div> : <div className="grid gap-4 xl:grid-cols-2">{drivers.map((driver) => { const name = driver.profiles?.full_name || "Estafeta"; const phone = driver.phone || driver.profiles?.phone; const completedCount = driver.deliveries.filter((delivery) => delivery.status === "delivered").length; return <Card key={driver.id} className="border-zinc-200 shadow-none"><CardHeader><div className="flex items-start gap-4"><div className="flex size-12 items-center justify-center rounded-2xl bg-zinc-950 text-white"><UserRound className="size-5" /></div><div className="min-w-0 flex-1"><CardTitle className="truncate text-lg">{name}</CardTitle><div className="mt-2 flex flex-wrap items-center gap-2"><Badge variant="outline" className={statusClasses[driver.status]}>{statusLabels[driver.status]}</Badge>{!driver.is_active && <Badge variant="outline">Acesso suspenso</Badge>}</div></div><Button variant="outline" size="sm" disabled={pending || driver.status === "busy"} onClick={() => toggleDriver(driver)}><Power className="mr-1 size-4" />{driver.is_active ? "Suspender" : "Reativar"}</Button></div></CardHeader><CardContent className="space-y-4"><div className="grid grid-cols-2 gap-3 rounded-2xl bg-zinc-50 p-4"><div><p className="text-xs text-zinc-500">Veículo</p><p className="mt-1 flex items-center gap-1.5 font-medium"><Truck className="size-4" />{driver.vehicle_type ?? "Não indicado"}</p></div><div><p className="text-xs text-zinc-500">Concluídas</p><p className="mt-1 font-medium">{completedCount}</p></div></div>{phone && <a href={`tel:${phone}`} className="flex items-center gap-2 text-sm text-zinc-600"><Phone className="size-4" />{phone}</a>}{driver.location_updated_at && <p className="flex items-center gap-2 text-xs text-zinc-400"><MapPin className="size-3.5" /> Localização atualizada às {new Intl.DateTimeFormat("pt-PT", { hour: "2-digit", minute: "2-digit" }).format(new Date(driver.location_updated_at))}</p>}</CardContent></Card>; })}</div>}
    </div>
  );
}
