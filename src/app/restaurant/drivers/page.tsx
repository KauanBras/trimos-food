...
export default async function RestaurantDriversPage() {
  ...
  const [driversResult, invitesResult, earningsResult] = await Promise.all([
    ...
    supabase.from("driver_earnings").select("id, driver_fee, cash_collected, net_balance, status, created_at, settled_at, drivers(id, payout_method, payout_phone, payout_iban, profiles(full_name)), orders(customer_name)").eq("restaurant_id", restaurantId).order("created_at", { ascending: false }),
  ]);
