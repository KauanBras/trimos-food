...
export default async function PublicCartPage({ params, searchParams }: Props) {
  ...
  const { data: restaurant } = await supabase.from("restaurants").select("id, name, slug, currency_code, accepts_delivery, accepts_pickup, accepts_dine_in, timezone").eq("slug", slug).eq("status", "active").maybeSingle();
