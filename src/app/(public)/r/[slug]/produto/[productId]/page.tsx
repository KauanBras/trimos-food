...
export default async function PublicProductPage({ params, searchParams }: Props) {
  ...
  return <PublicProductConfigurator restaurant={{ id: restaurant.id, slug: restaurant.slug, currencyCode: restaurant.currency_code }} product={{ id: product.id, name: product.name, description: product.description, imageUrl: product.image_url, price: product.price }} variants={(variants ?? []).map((variant) => ({ id: variant.id, name: variant.name, price: variant.price }))} groups={orderedGroups} tableCode={tableCode} />;
}
