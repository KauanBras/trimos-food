-- Archived catalogue rows remain recoverable but are no longer part of the
-- editable menu order.
...
begin
  ...
  if cardinality(requested_product_ids) <> expected_count
    ...
    raise exception 'A ordem enviada não corresponde aos produtos atuais deste restaurante.';
  end if;
