...
begin
  ...
  set
    ...
    accepted_at = case when requested_status = 'preparing' then coalesce(accepted_at, current_time) else accepted_at end,
    ready_at = case when requested_status = 'ready' then current_time else ready_at end,
    completed_at = case when requested_status = 'completed' then current_time else completed_at end,
    cancelled_at = case when requested_status = 'cancelled' then current_time else cancelled_at end,
    paid_at = case when next_payment_status = 'paid' then coalesce(paid_at, current_time) else paid_at end
